...
public class HomeController {
	...
	public String home(Model model ) {
		...
		String defaultCategory = "freePage"; // 기본 카테고리 설정
		Map<String, String> categoryImages = new HashMap<>();
		categoryImages.put("freePage", "/image/sotong.png");
		categoryImages.put("qnaPage", "/image/curious.png");
		categoryImages.put("studyPage", "/image/study.png");
//		categoryImages.put("프로젝트", "/image/project.png");

		model.addAttribute("categoryImages", categoryImages);
		List<BoardQna> top12PostsByCategory = homeService.getTop12PostsByCategory(defaultCategory); // 쿼리메소드 Cnt 기준 Top 12 DESC
...
//		model.addAttribute("top12Posts", top12PostsByCommentCount); //댓글 모델에 담음
	...
	public String homeWithCategory(@PathVariable(value="category", required=false) String category, Model model) {
		if (category == null) {
			category = "freePage"; // Set the default category
		}
		System.out.println("2q12321321");
		...
		List<BoardQna> top12PostsByCategory = homeService.getTop12PostsByCategory(category); // 쿼리메소드 Cnt 기준 Top 12 DESC

		System.out.println(top12PostsByCategory.toString());
		model.addAttribute("top12Posts", top12PostsByCategory); // 모델에담음
		return "/view/home"; //이 템플릿으로 간다.
	}
